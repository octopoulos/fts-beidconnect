...
public:
	...
	SCardException(LONG SCardResult, SCardException_Code code = SCardException_Code::Unknown)
	    : SCardResult(SCardResult)
	    , code(code)
	{
	}
...
public:
	...
	/// <param name="code">Functionnal code</param>
	CardException(uint16_t SW, CardException_Code code = CardException_Code::Unknown)
	    : SW(SW)
	    , code(code)
	{
	}
...
public:
	BeidConnectException(BeidConnectException_Code code)
	    : code(code)
	{
	}
...
{
public:
	CardFileException(const std::string& _Message)
	    : std::exception(/*_Message*/)
	{
	}
