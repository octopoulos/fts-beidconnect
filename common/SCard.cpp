...
void SCard::listReaders(std::vector<std::shared_ptr<CardReader>>& readers)
{
	...
	lReturn           = SCardListReaders(*context, NULL, (LPTSTR)&szReaders, &cchReaders);
#else
...
bool SCard::getPPDUFeatures()
{
	...
	if ((name.find("VASCO DIGIPASS 870") == 0) || (name.find("VASCO DIGIPASS 875") == 0) || (name.find("VASCO DIGIPASS 920") == 0) || (name.find("VASCO DIGIPASS 840") == 0) || (name.find("VASCO DIGIPASS 876") == 0) || (name.find("Gemalto ING Shield Pro") == 0) || (name.find("ETSWW eKrypto PINPhab") == 0) || (name.find("ETSWW eKrypto PINPad") == 0) || (name.find("DIOSS pinpad") == 0))
	{
		try
		{
	...
	if (!FeaturesRetrieved)
	{
		if (!getPPDUFeatures())
		{
			...
			ret = SCardControl(hCard, // XXXXX get handle without card if no card present!!!!!
			                   CM_IOCTL_GET_FEATURE_REQUEST, NULL, 0, buf, sizeof(buf), &rcv_len);
...
void SCard::verify_pinpad(unsigned char format, unsigned char PINBlock, size_t PINLength, uint16_t PINMaxExtraDigit, const unsigned char pinAPDU[], size_t l_pinAPDU, uint16_t* sw)
{
	...
	else
	{
		...
		send_buf[1] = 0xc2;
		send_buf[2] = 0x01;
		send_buf[3] = (unsigned char)verify_pin_cmd;
		send_buf[4] = (unsigned char)send_buf_len;
		send_buf_len += 5;
